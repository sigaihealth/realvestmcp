export class COCRCalculator {
  ...
  calculate(params) {
    ...
    return {
      ...
      cash_flow_analysis: {
        ...
        cash_flow_per_unit: annual_cash_flow // Assumes single unit, adjust if needed
      },
      return_metrics: {
        ...
        annual_roi: parseFloat(cocr.toFixed(2)), // Same as COCR for year 1
  ...
  performScenarioAnalysis(originalParams, totalCashInvested, baseCOCR) {
    ...
    vacancyRates.forEach(rate => {
      ...
      const result = this.calculate(modifiedParams);
      scenarios.push({
        type: 'Vacancy Rate',
